...
export async function startBot(): Promise<void> {
  ...
  const webhookUrl = env.telegramWebhookUrl ?? DEFAULT_WEBHOOK_URL;
  ...
  try {
    await bot.api.setChatMenuButton({
      menu_button: { type: 'web_app', text: '🎮 AI COACH', web_app: { url: webappUrl } },
    });
  } catch (err) {
  ...
  bot.command('profile', async (ctx) => {
    ...
    if (!user) { await ctx.reply('You have not started the app yet. Press /start.'); return; }
    const account = await findFaceitAccountByUserId(db, user.id);
    if (account) await ctx.reply(`FACEIT: connected\nNickname: ${account.nickname}\nSkill level: ${account.skillLevel ?? 'n/a'}`);
    else await ctx.reply('FACEIT: not connected yet.\nOpen the Mini App and press "Connect FACEIT".');
  });

  bot.command('matches', async (ctx) => {
    const telegramId = ctx.from!.id;
    const user = await findUserByTelegramId(db, telegramId);
    if (!user) { await ctx.reply('No matches yet — press /start then connect FACEIT.'); return; }
    const matches = await listMatchesForUser(db, { userId: user.id, limit: 10 });
    if (matches.length === 0) { await ctx.reply('No matches recorded yet. Finished matches will appear here after connecting FACEIT.'); return; }
  ...
  bot.command('settings', async (ctx) => {
    await ctx.reply(`Environment: ${env.nodeEnv}${env.isDemoMode ? ' (DEMO MODE)' : ''}\nMini App: ${webappUrl}\nDemo data is only available in non-production builds.`);
  });
  ...
  const server: Server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (req.url === '/telegram/webhook' && req.method === 'POST') {
      ...
      try { await webhookHandler(req, res); }
        ...
        logger.error('webhook_handler_failed', { error: err instanceof Error ? err.message : String(err) });
  ...
  const shutdown = async () => {
    ...
    try { await bot.api.deleteWebhook({ drop_pending_updates: false }); } catch (err) { logger.warn('webhook_delete_failed', { error: err instanceof Error ? err.message : String(err) }); }
    server.close();
    await closeDb();
  };
