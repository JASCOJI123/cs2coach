...
export async function buildServer(config:AppConfig=createAppConfig(), options:BuildServerOptions={}):Promise<FastifyInstance>{
  ...
  if (options.enableMiddlewarePlugins !== false) {
    ...
    app.addHook('onSend',async(request,reply)=>{const origin=String(request.headers.origin ?? '');if(isAllowedOrigin(origin || undefined)){if(origin) reply.header('access-control-allow-origin',origin);reply.header('access-control-allow-credentials','true');reply.header('access-control-allow-headers','Content-Type, Authorization, X-Telegram-Init-Data');reply.header('access-control-allow-methods','GET,POST,PUT,PATCH,DELETE,OPTIONS');}});
