...
export async function buildServer(config:AppConfig=createAppConfig()):Promise<FastifyInstance>{
  ...
  await app.register(rateLimit,{max:1000,timeWindow:'1 minute'}); await app.register(websocket);
  ...
  const shutdown=async(signal:string)=>{logger.info('shutdown',{signal});config.faceitMatchProvider.stopAll();config.demoProvider?.stop();await app.close();process.exit(0)}; process.on('SIGINT',()=>void shutdown('SIGINT')); process.on('SIGTERM',()=>void shutdown('SIGTERM')); return app;
}
