...
export async function matchesRoutes(app: FastifyInstance, config: AppConfig): Promise<void> {
  ...
  app.get('/api/matches/:faceitMatchId', { preHandler: await requireAuth(config) }, async (request, reply) => {
    ...
    // A fresh history item may be visible before its DB transaction completes.
    // Resolve it directly from FACEIT so opening a newly discovered match never 404s.
    const user = request.authedUser!;
    const account = await findFaceitAccountByUserId(config.db, user.userId);
    ...
    try {
      ...
      return reply.send({ ok: true, data: {
        id: `faceit-${detail.match_id}`,
        faceitMatchId: detail.match_id,
        map: detail.details?.map ?? null,
        status: detail.status ?? 'finished',
        score: { a: detail.results?.score?.faction1 ?? 0, b: detail.results?.score?.faction2 ?? 0 },
        live: null,
      } });
    } catch {
