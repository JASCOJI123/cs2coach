...
export async function matchAnalysisRoutes(app: FastifyInstance, config: AppConfig): Promise<void> {
  app.get('/api/matches/:faceitMatchId/analysis', { preHandler: await requireAuth(config) }, async (request, reply) => {
    ...
    const account = await findFaceitAccountByUserId(config.db, user.userId); const match = await getMatchByFaceitId(config.db, faceitMatchId);
  ...
  app.post('/api/matches/:faceitMatchId/analysis', { preHandler: await requireAuth(config) }, async (request, reply) => {
    ...
    const [playerRows, roundRows, patternRows] = await Promise.all([
      config.db`SELECT p.nickname, mp.player_id, COALESCE(ps.kills, mp.kills, 0) AS kills, COALESCE(ps.deaths, mp.deaths, 0) AS deaths, COALESCE(ps.assists, mp.assists, 0) AS assists, COALESCE(ps.opening_kills, 0) AS opening_kills, COALESCE(ps.opening_deaths, 0) AS opening_deaths, COALESCE(ps.utility_damage, 0) AS utility_damage, COALESCE(ps.flash_assists, 0) AS flash_assists, COALESCE(ps.clutches, 0) AS clutches FROM match_players mp JOIN players p ON p.id = mp.player_id LEFT JOIN player_statistics ps ON ps.match_id = mp.match_id AND ps.player_id = mp.player_id JOIN faceit_accounts fa ON fa.faceit_user_id = p.faceit_player_id WHERE mp.match_id = ${match.id} AND fa.user_id = ${user.userId} LIMIT 1`,
    ...
    const result = await new PostMatchAnalysisService(config.groqClient).generate({ map: match.map ?? null, finalScore: { a: Number(match.scoreA ?? 0), b: Number(match.scoreB ?? 0) }, player: { nickname: String(player.nickname), kills: Number(player.kills), deaths: Number(player.deaths), assists: Number(player.assists), openingKills: Number(player.opening_kills), openingDeaths: Number(player.opening_deaths), utilityDamage: Number(player.utility_damage), flashAssists: Number(player.flash_assists), clutches: Number(player.clutches) }, rounds, opponentPatterns });
