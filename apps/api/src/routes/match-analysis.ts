...
export async function matchAnalysisRoutes(app: FastifyInstance, config: AppConfig): Promise<void> {
  ...
  app.post('/api/matches/:faceitMatchId/analysis', { preHandler: await requireAuth(config) }, async (request, reply) => {
    ...
    const [playerRows, roundRows, patternRows] = await Promise.all([
      config.db`SELECT p.nickname, mp.player_id, COALESCE(ps.kills, mp.kills, 0) AS kills, COALESCE(ps.deaths, mp.deaths, 0) AS deaths, COALESCE(ps.assists, mp.assists, 0) AS assists, COALESCE(ps.opening_kills, 0) AS opening_kills, COALESCE(ps.opening_deaths, 0) AS opening_deaths, COALESCE(ps.utility_damage, 0) AS utility_damage, COALESCE(ps.flash_assists, 0) AS flash_assists, COALESCE(ps.clutches, 0) AS clutches FROM match_players mp JOIN players p ON p.id = mp.player_id LEFT JOIN player_statistics ps ON ps.match_id = mp.match_id AND ps.player_id = mp.player_id JOIN faceit_accounts fa ON fa.faceit_player_id = p.faceit_player_id WHERE mp.match_id = ${match.id} AND fa.user_id = ${user.userId} LIMIT 1`,
    ...
    try {
      syncedStats = await syncFaceitPlayerMatchStats(
        config,
        match.id,
        String(player.player_id),
        account.faceitUserId,
        faceitMatchId,
      );
      if (!syncedStats) config.logger.warn('faceit_match_player_stats_missing', { faceitMatchId, faceitPlayerId: account.faceitUserId });
    } catch (error) {
    ...
    const effectivePlayer = {
      ...
      kills: syncedStats?.kills ?? Number(player.kills),
      deaths: syncedStats?.deaths ?? Number(player.deaths),
      assists: syncedStats?.assists ?? Number(player.assists),
      openingKills: syncedStats?.openingKills ?? Number(player.opening_kills),
      openingDeaths: syncedStats?.openingDeaths ?? Number(player.opening_deaths),
      utilityDamage: syncedStats?.utilityDamage ?? Number(player.utility_damage),
      flashAssists: syncedStats?.flashAssists ?? Number(player.flash_assists),
      clutches: syncedStats?.clutches ?? Number(player.clutches),
    };
    ...
    await config.db.begin(async (sql) => {
      await sql`INSERT INTO match_analysis (match_id, post_match_analysis) VALUES (${match.id}, ${sql.json(result.analysis)}) ON CONFLICT (match_id) DO UPDATE SET post_match_analysis = EXCLUDED.post_match_analysis, updated_at = now()`;
      await sql`INSERT INTO training_plans (user_id, match_id, plan_json) VALUES (${user.userId}, ${match.id}, ${sql.json(result.analysis.trainingPlan)}) ON CONFLICT (user_id, match_id) DO UPDATE SET plan_json = EXCLUDED.plan_json`;
    });
