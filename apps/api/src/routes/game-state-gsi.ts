...
import { addMatchPlayer, findFaceitAccountByFaceitUserId, listMatchesForUser, upsertMatchFromFaceit, upsertPlayer } from '@cs2coach/database';
...
export async function gameStateGsiRoutes(app: FastifyInstance, config: AppConfig): Promise<void> {
  app.post('/api/game-state/gsi', { config: { rateLimit: false } }, async (request, reply) => {
