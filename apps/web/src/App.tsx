...
export default function App() {
  ...
  if (current.route !== 'splash' && current.route !== 'faceit-callback' && !hasAuthToken()) {
    ...
    switch (current.route) {
      ...
      case 'matches': content = <Matches />; break;
      case 'match': content = current.param ? <MatchPage key={current.param} faceitMatchId={current.param} /> : <Matches />; break;
      case 'post-match': content = current.param ? <PostMatch key={current.param} faceitMatchId={current.param} /> : <Matches />; break;
      case 'faceit-callback': content = <FaceitCallback />; break;
      default: content = <Splash />;
    }
  }
