...
export function setAuthToken(token: string): void { authToken = token; try { if (token) window.localStorage.setItem(AUTH_STORAGE_KEY, token); else window.localStorage.removeItem(AUTH_STORAGE_KEY); } catch {} }
export function clearAuthToken(): void { setAuthToken(''); }
export function hasAuthToken(): boolean { return Boolean(authToken || loadStoredToken()); }
export function authTokenOrThrow(): string { if (!authToken) authToken = loadStoredToken(); if (!authToken) throw new ApiError('Not authenticated', 401); return authToken; }
const productionApiUrl = 'https://cs2coach-api.onrender.com';
export const apiUrl = (path: string): string => (import.meta.env.VITE_API_URL || productionApiUrl) + path;
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  if (!authToken) authToken = loadStoredToken();
  const headers: Record<string, string> = { ...(init.headers as Record<string, string> | undefined) };
  if (init.body != null && !headers['content-type'] && !headers['Content-Type']) headers['content-type'] = 'application/json';
...
export const api = {
  ...
  getMatch: (faceitMatchId: string) => request<{ id: string; faceitMatchId: string; map: string | null; status: string; score: { a: number; b: number }; live: MatchStateLite | null }>(`/api/matches/${encodeURIComponent(faceitMatchId)}`),
  requestCoach: (faceitMatchId: string) => request<TacticalDecisionLite>(`/api/matches/${encodeURIComponent(faceitMatchId)}/coach`, { method: 'POST' }),
  getMatchAnalysis: (faceitMatchId: string) => request<PostMatchAnalysis | null>(`/api/matches/${encodeURIComponent(faceitMatchId)}/analysis`),
  generateMatchAnalysis: (faceitMatchId: string) => request<PostMatchAnalysis>(`/api/matches/${encodeURIComponent(faceitMatchId)}/analysis`, { method: 'POST', body: '{}' }),
