...
export interface FaceitPlayerRef {
...
export interface FaceitTeamRef {
  teamId?: string;
  name?: string;
  players: FaceitPlayerRef[];
}

// ── game events (normalized pipeline input, spec §22/§24) ──────────────────

export interface MatchNameEvent {
  matchId: string;
  ts: number;
}

export interface TeamScoreEvent extends MatchNameEvent {
  scoreA: number;
  scoreB: number;
}
...
export type GameEvent =
  ...
  | ({
      type: 'player_connect';
      player: FaceitPlayerRef;
      team: 'A' | 'B';
    } & MatchNameEvent)
  | ({
      type: 'player_kill';
      round: number;
      killer: FaceitPlayerRef;
      victim: FaceitPlayerRef;
      weapon?: string;
      headshot?: boolean;
      site?: 'A' | 'B';
    } & MatchNameEvent)
...
export interface RoundResult {
  ...
  economyAtStart?: { a: TeamEconomy; b: TeamEconomy };
  executedAction?: ActionType;
}

// ── learned patterns (spec §27) ─────────────────────────────────────────────

export interface OpponentPattern {
  playerId?: string;
  player?: FaceitPlayerRef;
  patternType: 'position' | 'site' | 'aggression' | 'utility' | 'rotation' | 'clutch' | 'execute';
  location: string;
...
export interface MatchState {
  ...
  side?: 'CT' | 'T';
  players: MatchPlayer[];
  alivePlayers: MatchPlayer[];
  economy?: EconomyState;
  bomb: BombsiteInfo;
  positions: Record<string, PositionSample[]>;
  mapControl?: { a?: number; b?: number; mid?: number };
  recentEvents: GameEvent[];
  previousRounds: RoundResult[];
  opponentPatterns: OpponentPattern[];
  teamPatterns: TeamPattern[];
  veto: VetoStep[] | null;
  teams?: { a?: FaceitTeamRef; b?: FaceitTeamRef };
  /** Deterministic hash of tactical state — if unchanged, AI must not re-run (spec §54). */
  stateVersion: number;
  stateHash: string;
  timestamp: number;
  /** True when a legitimate real-time CS2 game-state source is connected. */
  gameDataAvailable: boolean;
  aiStatus: AiStatus;
  /** True only inside local demo mode. Production never sees demo events. */
  demoMode: boolean;
}

// ── coaching output (spec §20/§21/§30) ──────────────────────────────────────
...
export interface Recommendation {
  ...
  timing?: string;
  utility?: string;
  confidence: number; // 0..1, no fake precision
  reason: string;
  warning?: string;
  role?: PlayerRole;
  playerInstructions: PlayerInstruction[];
  source: 'groq' | 'tactical-engine';
  generatedAt: number;
}

// ── analysis (spec §17/§41/§42) ─────────────────────────────────────────────

export interface PreMatchAnalysis {
  matchId: string;
  map?: string;
  winProbability?: { a: number; b: number; confidence: Confidence };
  teamStrengths: { team: 'A' | 'B'; items: string[] };
  teamWeaknesses: { team: 'A' | 'B'; items: string[] };
  playerRoles: { playerId: string; nickname: string; role: PlayerRole }[];
  mapAnalysis: string[];
  tacticalPlan: string[];
...
export interface PerformanceMetric {
  name: string; // Aim, Positioning, Decision Making, Utility, Trading, Opening, Clutch, Teamplay
  score: number; // 0..100
  comment: string;
}
...
// ── summaries for lists (spec §10 match cards) ──────────────────────────────
