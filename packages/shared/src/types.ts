...
export interface MatchState {
  ...
  players: MatchPlayer[]; alivePlayers: MatchPlayer[]; economy?: EconomyState; bomb: BombsiteInfo; positions: Record<string, PositionSample[]>; mapControl?: { a?: number; b?: number; mid?: number };
  recentEvents: GameEvent[]; previousRounds: RoundResult[]; opponentPatterns: OpponentPattern[]; teamPatterns: TeamPattern[]; veto: VetoStep[] | null; teams?: { a?: FaceitTeamRef; b?: FaceitTeamRef };
  stateVersion: number; stateHash: string; timestamp: number; gameDataAvailable: boolean; aiStatus: AiStatus; demoMode: boolean;
}
...
export function emptyMatchState(matchId: string): MatchState { return { matchId, status: 'ready', phase: 'pre_match', score: { a: 0, b: 0 }, round: 0, players: [], alivePlayers: [], bomb: { planted: false }, positions: {}, opponentPatterns: [], teamPatterns: [], veto: null, recentEvents: [], previousRounds: [], stateVersion: 0, stateHash: '', timestamp: Date.now(), gameDataAvailable: false, aiStatus: 'READY', demoMode: false }; }
