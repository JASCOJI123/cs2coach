...
function roundOf(body: any): number { return Math.max(1, n(body?.map?.team_ct?.score) + n(body?.map?.team_t?.score) + 1); }
export function normalizeGsiSnapshot(body: unknown, previous?: GsiSnapshot): GameEvent[] | GsiSnapshot {
  const b: any = body && typeof body === 'object' ? body : {};
  const current: GsiSnapshot = { phase: typeof b.round?.phase === 'string' ? b.round.phase : undefined, round: roundOf(b), scoreCT: n(b.map?.team_ct?.score), scoreT: n(b.map?.team_t?.score), bomb: typeof b.bomb?.state === 'string' ? b.bomb.state : undefined, health: n(b.player?.state?.health, 0), steamId: typeof b.player?.steamid === 'string' ? b.player.steamid : undefined };
  if (!previous) return current;
  const events: GameEvent[] = [];
  const matchId = String(b.provider?.matchid ?? b.map?.name ?? 'gsi');
  const ts = Date.now();
